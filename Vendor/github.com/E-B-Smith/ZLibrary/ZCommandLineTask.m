...
- (id) initWithCommandPath:(NSString*)commandLine
	...
	BNCLogDebug(@"CommandLineTask: %@ Command: %@", self, commandLine);
	BNCLogAssert([NSThread isMainThread]);	//	Must be started on main thread to get notifications.
	...
	if ([argumentsArray count] == 0)
		{
		BNCLogError(@"initWithCommand called with empty command line.");
		return nil;
		}
...
- (void) pipeEOF:(NSNotification*)n
	...
	if (p == errLine)
		...
		{
		BNCLogWarning(@"Recieved EOF on unknown pipe.");
		}
...
- (void) taskTerminatedNote:(NSNotification*)n
	{
	BNCLogDebug(@"Task terminated notification.");
...
- (void) processTaskLine:(NSString*)taskLine
	...
	BNCLogDebug(@"Task line: %@", taskLine);
...
- (void) processErrorLine:(NSString*)errorLine 
	...
	BNCLogDebug(@"Error line: %@", errorLine);
...
- (void) taskTerminated:(int)status 
	...
	BNCLogDebug(@"Task terminated, status: %d", status);
